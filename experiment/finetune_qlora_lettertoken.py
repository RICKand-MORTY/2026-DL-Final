...
- Train with CE over these K option scores; infer by argmax.
"""
...
def option_letter_logprobs(
    ...
    """
    Compute (K,) scores using ONLY the next-token log-probabilities of letters.
    """
    ...
    if logits.dtype in (torch.float16, torch.bfloat16):
        logits = logits.float()
    # logits at last position predicts the next token after "Answer:"
...
def main() -> None:
    ...
    p.add_argument("--output_dir", type=str, default="outputs/qlora_lettertoken")
    p.add_argument("--submission_file", type=str, default="my_submission_lettertoken.csv")
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to saved PEFT adapter (e.g. outputs/qlora_lettertoken/adapter).",
    )
    args = p.parse_args()
