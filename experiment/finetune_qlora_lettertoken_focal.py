"""
QLoRA + letter-token-only scoring for ScienceQA-style MCQ (focal variant).

Scheme:
- Build prefix ending with "Answer:" (without gold label).
- Run ONE forward pass per sample.
- Use only the next-token distribution at the last prefix position.
- Score each option by log P(letter_token | image, prefix).
- Train with focal loss over these K option scores; infer by argmax.
"""
...
def run_train_focal(
    ...
    print(
        f"Using focal loss with gamma={focal_gamma}, alpha={focal_alpha}. "
    ...
    for epoch in range(num_epochs):
        ...
        pbar = tqdm(
            ...
            total=num_micro,
            desc=f"Epoch {epoch+1}/{num_epochs} (letter-token focal)",
        )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="QLoRA letter-token training/inference with focal loss."
    )
    p.add_argument("--data_dir", type=str, default="data")
    p.add_argument("--model_id", type=str, default=MODEL_ID)
    p.add_argument("--output_dir", type=str, default="outputs/qlora_lettertoken_focal")
    p.add_argument(
        "--submission_file",
        type=str,
        default="my_submission_lettertoken_focal.csv",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=2.0,
        help="Focal gamma; 0.0 becomes plain CE.",
    )
    p.add_argument(
        "--focal_alpha",
        type=float,
        default=1.0,
        help="Focal alpha scaling; set <0 to disable alpha scaling.",
    )
    p.add_argument(
        "--test_only",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to saved PEFT adapter (e.g. outputs/qlora_lettertoken_focal/adapter).",
    )
    args = p.parse_args()
