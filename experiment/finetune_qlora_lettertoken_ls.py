"""
QLoRA + letter-token-only scoring for ScienceQA-style MCQ (label smoothing).
...
- Train with cross-entropy + optional label smoothing over K option scores; infer by argmax.
"""
...
def run_train(
    ...
    for epoch in range(num_epochs):
        ...
        pbar = tqdm(
            ...
            total=num_micro,
            desc=f"Epoch {epoch+1}/{num_epochs} (letter-token CE+LS)",
        )
        ...
        for start in pbar:
            ...
            for idx in mb:
                ...
                y = int(row["answer"])
                k = int(row["num_choices"])
                if label_smoothing > 0 and k < 2:
                    raise ValueError("label_smoothing>0 requires num_choices >= 2.")
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="QLoRA letter-token training/inference with label smoothing."
    )
    p.add_argument("--data_dir", type=str, default="data")
    p.add_argument("--model_id", type=str, default=MODEL_ID)
    p.add_argument("--output_dir", type=str, default="outputs/qlora_lettertoken_ls")
    p.add_argument(
        "--submission_file",
        type=str,
        default="my_submission_lettertoken_ls.csv",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="0 = plain CE. Typical: 0.05–0.1. Must be in [0,1).",
    )
    p.add_argument(
        "--test_only",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to saved PEFT adapter (e.g. outputs/qlora_lettertoken_ls/adapter).",
    )
    args = p.parse_args()
